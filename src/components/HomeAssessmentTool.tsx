...
interface HomeAssessmentToolProps {
  /** Show the full page heading + description (default true) */
  showHeading?: boolean;
  /** Called when the tool finishes, with the answers */
  onComplete?: (answers: Record<string, number>) => void;
}

export function HomeAssessmentTool({ showHeading = true, onComplete }: HomeAssessmentToolProps) {
  ...
  if (done) {
    ...
    return (
      <div>
        {showHeading && (
          <div className="mb-6">
            <h2 className="font-serif text-2xl font-bold" style={{ color: '#1B4332' }}>
              Your Home Safety Plan
            </h2>
            <p className="text-gray-500 mt-1">Based on your answers, here are the categories most relevant for you:</p>
          </div>
        )}
        ...
        {recs.length > 0 ? (
          ...
          <div className="mb-6 p-4 rounded-xl bg-green-50 border border-green-200">
            <p className="font-semibold" style={{ color: '#1B4332' }}>Your home sounds well-prepared.</p>
            <p className="text-sm text-gray-600 mt-1">
              Consider a preventive assessment to stay ahead of future needs.
            </p>
          </div>
        )}
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-4">
          ...
          <p className="text-sm text-amber-800 mt-1 mb-3">
            A CAPS-certified contractor can do a free in-home assessment.
          </p>
          <Link
            href="/contractors"
